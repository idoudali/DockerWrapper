...
FOLDERS = [
    os.path.realpath(os.path.join(i, ".."))
...
for f in FOLDERS:
    spec = importlib.util.spec_from_file_location(
        "docker_wrapper_extensions", os.path.join(f, "docker_wrapper_extensions.py")
    )
...
def build_image(docker_client, image_name, tag, docker_file_path):
    """Build the image"""
    cmd = ["docker", "build", docker_file_path, "-t", "{}:{}".format(image_name, tag)]
    print(cmd)
...
def run(args):
    ...
    cmd += ["-v", home_dir + ":" + home_dir, "-v", f"{project_path}:{project_path}", image]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prompt", action="store_true", help="Get a prompt inside the container for the project"
    )
    parser.add_argument(
        "--enable-nvidia",
        action="store_true",
    ...
    parser.add_argument(
        "--network", default=None, help="Pass through option to the " "docker run network option"
    )
    parser.add_argument("--privileged", action="store_true", help="Enable privileged mode")
    parser.add_argument("--enable-gui", action="store_true", help="Enable start gui apps")
    parser.add_argument("--port", default="", help="Port to forward from docker")
